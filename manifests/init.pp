#
# @param ignoreip    Array of IP that must not be banned.  #
# @param bantime     Time in second after with banned IP is removed from blacklist.
# @param findtime    Time interval in seconds during which *maxretry* must be reached.
# @param maxretry    Number of attempts within *findtime*.
# @param backend     Backend to be used to detect changes in the logpath.
...
#
# @param logencoding  Encoding of log files used for decoding. Default value of "auto" uses current system locale.
#
# @param destemail
...
#
# @param sender    E-mail from which fail2ban will send notifications.
# @param mta       MTA for the mailing.
# @param protocol  Default protocol.
# @param chain     iptables chain to insert rules.
# @param port      Default port.
# @param banaction Banning action (default iptables-multiport) typically specified in the [DEFAULT] section for all jails.
#   Can be redefined in separately for each jail.
# @param banaction_allports   The same as banaction but for some "allports" jails like "pam-generic" or "recidive" (default iptables-allports).
#
class fail2ban (
